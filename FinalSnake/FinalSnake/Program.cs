    ...
    class Program
    {
        ...
        static void Menu()
        {
            while (true)
            {
                ...
                {
                    Console.BackgroundColor = (pos == i) ? ConsoleColor.Cyan : ConsoleColor.Black;
                ...
                ConsoleKeyInfo btn = Console.ReadKey();
                if (btn.Key == ConsoleKey.UpArrow) pos = ((pos == 0) ? pos = 2 : --pos);
                if (btn.Key == ConsoleKey.DownArrow) pos = ((pos == 2) ? pos = 0 : ++pos);
        ...
        static GameSave Get() {
            FileStream fs = new FileStream(@"data1.ser", FileMode.Open, FileAccess.Read);
            BinaryFormatter xs = new BinaryFormatter();
